...
class SummaryWriterCallback(BaseCallback):
    ...
    def _on_step(self):
        ...
        if self.num_timesteps >= self._log_freq and self.num_timesteps >= self.next_logging_point:
            ...
            print(f"ASR: {ASR}, completed: {completed}, DO: {DO}, TO: {TO}")
            if ASR > self.best_ASR:
                self.best_ASR = ASR
                self.model.save(self.model_save_dir + f"{self.exp_name}_checkpoint.zip")
                print(f"Best_Model saved with ASR: {ASR}")
            ...
            self.tb_formatter.writer.add_scalar("test/best_ASR", self.best_ASR, self.next_logging_point)
