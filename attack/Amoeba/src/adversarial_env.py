...
from utils import action2packet, smoothed
...
class AdvEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.use_CertTA = use_CertTA
        self.least_inserting_size = 40 / self.MAX_UNIT
        if action_mode == "direction_limited" or action_mode == "addition_only":
            ...
            min_actions = [-1, 0]
        elif action_mode == "padding_and_addition":
            print("padding_and_addition")
            min_actions = [-0.5, 0]
    ...
    def get_test_generator(self):
        ...
        test_set = DataLoader(VariableRecordDataset(X_path, y_path, self.MAX_UNIT, self.MAX_DELAY, target=-1),
                              batch_size=1, shuffle=False, drop_last=True)
        while True:
            for x, _ in test_set:
                yield x
    ...
    def step(self, action: np.ndarray):
        ...
        self.step_c += 1
        terminated = False
        directional_action = action2packet(self.action_mode, self.remaining_bytes, self.last_t, action, least_inserting_size=self.least_inserting_size)

        # print("self.action_mode", self.action_mode)
        # print("action", action)
        # print("directional_action", directional_action)
        
        info["new_action"] = None
        # gen reward
        sign = 1 if self.remaining_bytes > 0 else -1
        signed_action = directional_action.copy()
        if self.action_mode == "padding_and_addition":
            signed_action[0] = sign * abs(directional_action[0])
            # assert abs(signed_action[0]) <= 0.5
            if action[0] < 0: # inserting
                info["new_action"] = {
                    "action": "inserting",
                    "value": int(np.round(np.abs(directional_action[0]) * self.MAX_UNIT).astype(int)),
                    "added_delay": float((self.last_t + action[1])* self.MAX_DELAY)
                }
            else:
                new_pkt_len = (np.abs(np.clip(np.abs(action[0]) + np.abs(self.remaining_bytes), -1, 1)))
                org_pkt_len = np.abs(self.remaining_bytes)
                assert new_pkt_len >= org_pkt_len
                info["new_action"] = {
                    "action": "padding",
                    "value": int(np.round((new_pkt_len - org_pkt_len) * self.MAX_UNIT).astype(int)),
                    "added_delay": float(action[1]* self.MAX_DELAY)
                }
        elif self.action_mode == "addition_only":
            new_pkt_len = np.abs(np.clip(sign * action[0] + self.remaining_bytes, -1, 1))
            org_pkt_len = np.abs(self.remaining_bytes)
            info["new_action"] = {
                    "action": "padding",
                    "value": int(np.round((new_pkt_len - org_pkt_len) * self.MAX_UNIT).astype(int)),
        ...
        self.generated_pkts.append(torch.tensor(signed_action, device=self.device, dtype=torch.float32))
        ...
        reward = self.get_reward(pkt_diff, added_delay, use_CertTA=self.use_CertTA)

        # gen next state
        next_pkt = torch.zeros(2)
        if self.remaining_bytes > 0:
            if self.remaining_bytes - pkt_size > 0:
                if pkt_size > 0 and self.action_mode == "padding_and_addition":
                    raise ValueError("Packet size should be negative" + str(self.remaining_bytes) + " " + str(directional_action)) 
                self.truncate_num += 1
            ...
            if self.remaining_bytes - pkt_size < 0:
                if pkt_size < 0 and self.action_mode == "padding_and_addition":
                    raise ValueError("Packet size should be positive" + str(self.remaining_bytes) + " " + str(directional_action))
                self.truncate_num += 1
    ...
    def get_reward(self, pkt_diff, added_delay, use_CertTA=False):
        # compute reward
        with torch.no_grad():
            ...
            if isinstance(self.discriminator, SDAE):
                ...
                if self.use_CertTA:
                    clf_result = smoothed(self.discriminator, generated_flow, self.MAX_UNIT)
                else:
