...
def action2packet(action_mode, last_pkt, last_delay, action, least_inserting_size=0.1):
    pkt = np.zeros((2,))
    ...
    if action_mode == "direction_limited":
        ...
        if action[0] < 0:
            n_size = np.sign(action[0]) * (max(least_inserting_size , np.abs(action[0])))
            # n_size = np.sign(action[0]) * (least_inserting_size + np.abs(action[0]))
            pkt[0] = sign * n_size
            pkt[1] = action[1] + last_delay
        else:
            pkt[0] = sign * action[0] + last_pkt
            pkt[1] = action[1] + last_delay
        return pkt
    else:
...
def smoothed(discriminator, batch_x, Max_Unit, batch_size = 1, batch_x_lengths = None):
    whole_res = []
    final_res = []
    for i in range(batch_size):
        whole_res.append([])
    for i in range(10):
        input_batch_x = smooth_sample(batch_x, Max_Unit)
        # print(input_batch_x - batch_x)
        batch_pred = discriminator(input_batch_x, batch_size, batch_x_lengths)
        for j in range(batch_size):
            whole_res[j].append(batch_pred[j])
    for i in range(batch_size):
        identity_res = whole_res[i]
        pos_elems = [item for item in identity_res if item > 0.5]
        neg_elems = [item for item in identity_res if item <= 0.5]
        if len(pos_elems) > len(neg_elems):
            final_res.append(1)
        else:
            final_res.append(0)
    return torch.tensor(final_res, dtype=torch.float32, device=batch_x.device)