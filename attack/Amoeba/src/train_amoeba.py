...
from utils import parse_data_config, Timer, smoothed
import numpy as np
...
def init_discriminator_optimizer(args):
    ...
    if args.dis == "dt":
        ...
        discriminator = DFDiscriminator(args.device, 2, args.MAX_UNIT)
    elif args.dis == "sdae":
    ...
    if args.dis in nn_dis:
        ...
        dis_optimizer = optim.AdamW(params=discriminator.parameters(), lr=1e-3, weight_decay=1e-4)
    ...
    for epoch_i in range(args.train_epochs):
        ...
        for x, y in tqdm(train_dataset):
            ...
            if batch_count == batch_size:
                # batch_x = smooth_sample(batch_x, args.MAX_UNIT)
    ...
    with torch.no_grad():
        for x, y in tqdm(dataset):
            ...
            if args.dis == "sdae":
                ...
                if args.use_CertTA:
                    pred = (smoothed(dis, x, args.MAX_UNIT)).cpu().numpy()
                else:
...
def post_process_flow(flow, max_unit, max_delay):
    direction_length = [int(np.round(item[0] * max_unit).astype(int)) for item in flow]
    delay = [item[1] * max_delay for item in flow]
    timestamp = []
    last_t = 0
    for i in delay:
        last_t += i
        timestamp.append(last_t)
    return {"direction_length": direction_length, "timestamp": timestamp}
    


def evaluate(env, model, episodes, discriminator, args, mode="val"):
    record_dir = args.log_dir + "records/"
    if not os.path.isdir(record_dir):
        os.mkdir(record_dir)
    record_file = record_dir + mode + ".json"
    actions_seq = []
    ...
    for i in tqdm(range(episodes)):
        ...
        while True:
            ...
            # actions.append({
            #     "action": "padding" if action[0] >= 0 else "inserting",
            #     "value": float(abs(action[0])),
            #     "added_delay": float(action[1])
            # })
            state, reward, done, info = env.step(action)
            actions.append(info["new_action"])
            ...
            if done:
                ...
                actions_seq.append({
                    "original_flow": post_process_flow(np.array(original_flow).tolist(), args.MAX_UNIT, args.MAX_DELAY),
                    "virtual_adv_flows": post_process_flow(torch.concat(virtual_adv_flows, dim=0).cpu().numpy().tolist(), args.MAX_UNIT, args.MAX_DELAY),
                    "actions": actions
                })
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_penalty", type=float, default=8, help="data penalty")
    parser.add_argument("--truncate_penalty", type=float, default=0.1, help="truncate penalty")
    parser.add_argument("--time_penalty", type=float, default=1, help="time penalty")
    parser.add_argument("--MAX_UNIT", type=int, default=3000, help="supports {1448, 16500}. 1448: tor, 16500: v2ray")
    ...
    parser.add_argument("--adv_iat_clip", type=float, default=0.1,
                        help="per-packet added delay is limited within 0.1")
    ...
    parser.add_argument("--padding_buget", type=int, default=100, help="padding budget")
    parser.add_argument("--use_CertTA", type=bool, default=False, help="use CertTA Discriminator")
    # parser.add_argument("--insertion_buget", type=int, default=3, help="insertion budget")
    parser.add_argument("--delay_buget", type=float, default=0.02, help="action budget")
    parser.add_argument("--basemodel", type=str, default="DF", help="base model for CertTA")
    parser.add_argument(
        "--action_mode",
        type=str,
    ...
    # parse data config
    args.data_config = args.ROOT + "config/" + args.dataset + ".json"
    args.encoder_path = args.ROOT + "saved_models/" + "encoder.pth"
    args.test_num = 600 if args.dataset == "CICDOH20" else 480
    args.trained_dis_path = args.ROOT + "saved_models/" + args.dataset + "_trained_dis.pth"
    ...
    print(args.device)
    args.log_dir = args.ROOT + "logs/" + args.exp_name + "_" + args.dataset + "/"
    if not os.path.isdir(args.log_dir):
        print("log dir not exist, create one.")
        os.mkdir(args.log_dir)

    args.model_save_dir = args.ROOT + "saved_models/"
    args.model_save_path = args.ROOT + "saved_models/" + args.exp_name + '_' + args.dataset + ".zip"
    ...
    # args.max_actions = (args.padding_buget/args.MAX_UNIT, args.delay_buget/args.MAX_DELAY)
    print("max actions: ", args.max_actions)
    single_env = AdvEnv(discriminator=discriminator, state_encoder=state_encoder,
                        ...
                        mask_rate=args.mask_rate, action_mode=args.action_mode, use_CertTA=args.use_CertTA)
    print("training PPO")
    vec_env = make_vec_env(AdvEnv, 4, env_kwargs=dict(discriminator=discriminator, state_encoder=state_encoder,
                                                      train_dataset=args.agent_train_dataset,
    ...
    # evaluate
    # args.best_model_path = args.ROOT + "saved_models/" + args.exp_name + "_checkpoint.zip"
    # model = PPO.load(args.best_model_path, env=single_env)
    model = PPO.load(args.model_save_path, env=single_env)
    print("testing the best_model")
