...
Based on the multi-modal robustness region derivation in Theorem 1 of our paper, we can collect the classification results saved by `test.py` to plot the CDF curves of certified accuracy under different robustness radius:
```bash
...
When the pre-defined lower bound threshold is 0.99, we select three attack intensities $(5, 0), (0, 619), (2, 556)$ to generate adversarial flows and evaluate baseline certification methods against these flows.
