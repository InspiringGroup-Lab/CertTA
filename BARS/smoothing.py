...
class Smooth2(Smooth):
    ...
    def _certify2_dl(self, x: list, n0: int, n: int, t: float, alpha: float, batch_size_memory: int) -> (np.ndarray, np.ndarray):
        """ Monte Carlo algorithm for certifying that g's prediction around x is constant within some L2 radius.
        With probability at least 1 - alpha, the class returned by this method will equal g(x), and g's prediction will
        robust within a L2 ball of radius R around x.
        ...
        self.base_classifier.eval()
        # draw samples of f(x+ epsilon)
        counts_selection = self._sample_noise2_dl(x, n0, t, batch_size_memory)
        # use these samples to take a guess at the top class
        cAHat = counts_selection.argmax(1)
        # draw more samples of f(x + epsilon)
        counts_estimation = self._sample_noise2_dl(x, n, t, batch_size_memory)
        # use these samples to estimate a lower bound on pA
        nA = counts_estimation[np.arange(cAHat.shape[0]), cAHat]
        pABar = self._lower_confidence_bound2(nA, n, alpha)
        # use pA lower bound to calculate normalized robustness radius
    ...
    def _sample_noise2(self, x: torch.tensor, num: int, t: float, batch_size_memory: int) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            while num > 0:
                ...
                batch = x.repeat((batch_size_per_example, 1, 1))
                noise = self.noise_generator.sample_feat(x.size(0) * batch_size_per_example) * t
                noise = noise.reshape((-1, batch.shape[1], batch.shape[2]))
                noised_batch = batch + noise

                score = self.base_classifier(noised_batch)
    ...
    def _sample_noise2_dl(self, x: list, num: int, t: float, batch_size_memory: int) -> np.ndarray:
        """ Sample the base classifier's prediction under noisy corruptions of the input x.

        :param x: the input [flow_length x packet_dim]
        :param num: number of samples to collect
        :param t: scale factor
        :param batch_size_memory: maximum batch size in memory for parallel smoothing
        :return: an ndarray[int] of length num_classes containing the per-class counts
        """
        # batch size for iteration should be less than or equal to maximum batch size in memory
        assert 1 <= batch_size_memory

        usemm = self.base_classifier.cfg.USEMM_FLAG
        classifier_device = self.base_classifier.cfg.DEVICE
        with torch.no_grad():
            counts = np.zeros((1, self.num_classes), dtype=int)
            while num > 0:
                batch_size_per_example = min(floor(batch_size_memory / 1), num)
                num -= batch_size_per_example

                batch_x_cache = None
                for _ in range(batch_size_per_example):
                    noise_feat = self.noise_generator.sample_feat(1) * t
                    noise_feat = noise_feat.reshape((1, -1, len(x[0][0])))
                    noise_feat = noise_feat[:, :len(x[0]), :]
                    input_X = copy.deepcopy(x)

                    if usemm:
                        (flow_mt_result1,flow_mt_result2,middle_result) = self.base_classifier.forward1_bars(input_X, noise_feat, classifier_device, usemm)
                    else:
                        middle_result = self.base_classifier.forward1_bars(input_X, noise_feat, classifier_device, usemm)
                ...
                score = self.base_classifier.forward2(batch_x_cache)

                _, predictions = torch.max(score, 1)
                # predictions = self.base_classifier(batch, noise)
                counts += self._count_arr2(predictions.cpu().numpy(), 1)
    ...
    def bars_certify_dl(self, x: list, n0: int, n: int, t: float, alpha: float, batch_size_memory: int) -> (np.ndarray, np.ndarray, np.ndarray):
        """ 
        :param x: the input [batch_size_iteration x feature_size]
        :param n0: the number of Monte Carlo samples to use for selection
        :param n: the number of Monte Carlo samples to use for estimation
        ...
        :return: (predicted class: np.ndarray, 
                 ...
                 in the case of abstention, the class will be ABSTAIN and the radius 0.
        """
        cAHat, radius_norm = self._certify2_dl(x, n0, n, t, alpha, batch_size_memory)
        radius_norm_dim = torch.tensor(radius_norm).unsqueeze(1).repeat((1, self.d)).to(self.device)
        radius_feat_dim = self.noise_generator.norm_to_feat(radius_norm_dim).cpu().numpy() * t
        radius_feat = radius_feat_dim.mean(1)
...
class Noise(object):
    ...
    def distribution_transformer_reverse(self, z: torch.tensor) -> torch.tensor:
        return self.distribution_transformer.reverse(z).to(self.device)